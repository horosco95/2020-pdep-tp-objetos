...
object mensajeria {
	...
	method enviar(unPaquete) {
		if (not self.algunoPuedeEntregar(unPaquete)) self.almacenarComoPendiente(unPaquete)
	}
...
object paqueton {
	...
	method puedeSerEntregadoPor(mensajero) = self.estaPago() && destinos.all({ destino => destino.dejaPasarA(mensajero) })

	method precio() = destinos.size() * 100

	method pagar(importe) = (pagado += importe)

	method destinos(nuevoDestino) = destinos.add(nuevoDestino)
